...
def macro_auprc(y_true: np.ndarray, prob: np.ndarray) -> float:
    """macro AUPRC over 3 classes: mean(one-vs-rest AP_k)."""
...
def macro_confusion_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> dict:
    """
    macro metrics over 3 classes using one-vs-rest per class,
...
def load_multi_ps(path: str) -> pd.DataFrame:
    ...
    if not USE_ARGMAX_PRED:
        ...
        if bad_p:
            raise ValueError(f"Pred_label 出现不在0~2的值：{sorted(list(bad_p))}。请检查。")
...
def main():
    ...
    out_path = os.path.join(os.path.dirname(FILE_PATH) if os.path.dirname(FILE_PATH) else ".",
                            "Multi_ps_age_gender_permutation_pvalues_macroMetrics.xlsx")
