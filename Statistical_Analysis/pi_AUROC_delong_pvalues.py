...
def load_multi_pi(path: str) -> pd.DataFrame:
    ...
    out = out.dropna(subset=["Number", "True_label", "Gender", "Age"])
    ...
    # sanity checks
    if set(np.unique(out["Gender"])) - {0, 1}:
        raise ValueError("Gender 不是0/1编码，请检查。")
    bad = set(np.unique(out["True_label"])) - set(range(N_CLASS))
    if bad:
        raise ValueError(f"True_label 出现不在0~7的值：{sorted(list(bad))}。请检查编码。")
...
def run_one_comparison(df: pd.DataFrame, mask1: np.ndarray, mask2: np.ndarray, name1: str, name2: str):
    """
    对比两组：逐类DeLong得到p，再Stouffer合并。
    返回：per_class_df, combined_row(dict)
    """
...
def main():
    ...
    for n1, n2, m1, m2 in comps:
        print(f"\n=== {n1} vs {n2}: per-class DeLong + Stouffer ===")
        per_df, combined = run_one_comparison(df, m1, m2, n1, n2)
        all_per.append(per_df)
        combined_rows.append(combined)
        print(f"  -> Stouffer: Z={combined['Z_Stouffer']:.4f}  p={combined['p_value_Stouffer_2sided']:.6g}  (classes_used={combined['Classes_used']})")

    df_per = pd.concat(all_per, ignore_index=True)
    df_comb = pd.DataFrame(combined_rows).sort_values("p_value_Stouffer_2sided")

    out_path = os.path.join(os.path.dirname(FILE_PATH) if os.path.dirname(FILE_PATH) else ".", "Multi_pi_age_gender_AUROC_DeLong_Stouffer.xlsx")
