...
test_imgs = np.load(".../ExternalData.npy")
test_imgs = test_imgs/255
y_pi_test = np.load(".../ExternalLabel_PI.npy")     #Isotype Label
y_pl_test = np.load(".../ExternalLabel_PS.npy")     #Severity Label
...
y_pl_test = np.array(y_pl_test); y_pi_test = np.array(y_pi_test)
...
#         self.dropout2 = nn.Dropout(0.1)
    ...
    def forward(self, x):
        ...
        output_pi = F.softmax(x_pi, dim=1)
...
checkpoint_vae = torch.load('.../ConVAE_IFE_0.pt')
checkpoint_net = torch.load('.../Net_IFE_0.pt')
...
with torch.no_grad():
    ...
    for i, (x, y_pl, y_pi) in enumerate(test_loader):
        ...
        output_pl, output_pi, x_plpi_weight = model(feature_extractor(x))
