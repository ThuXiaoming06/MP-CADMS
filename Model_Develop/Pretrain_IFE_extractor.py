...
pretrain_imgs = []
data_number = np.array([2, 3, 4])
for i in data_number:
    loaded = np.load(".../Pretrain_D" + str(i) + "_Ori" + ".npz")
    pretrain_imgs.append(loaded['imgs_arr'])

pretrain_imgs = np.concatenate(pretrain_imgs, axis=0)
print('pretrain_imgs.shape:', pretrain_imgs.shape)

transform_batch = transforms.Compose([transforms.ToTensor(), ])
pretrain_imgs = [transform_batch(img).type(torch.FloatTensor) for img in pretrain_imgs]
...
pretrain_dataset = CustomizedDataset(pretrain_imgs)
train_loader, valid_loader, num_train, num_valid = get_train_valid_loader(pretrain_dataset, 64, 666, collate_fn=my_collate)
